...
export const UploadScreen: React.FC = () => {
  ...
  const { isUploading, uploadProgress, uploadError, uploadedResume, pickAndUpload, reset: resetUpload } = useResumeUpload();
  const { isAnalyzing, analysisError, analyzeById } = useResumeAnalysis();

  const [phase, setPhase] = useState<UploadPhase>('idle');
  const [phaseMessage, setPhaseMessage] = useState('');

  // Pulsing upload zone animation
...
const progressStyle = {
  ...
  const handlePickAndUpload = async () => {
    ...
    if (!uploaded || !uploaded._id) {
      console.log("❌ Upload failed or empty");
      setPhase('error');
    ...
    if (!resumeId) {
      setPhase('error');
      setPhaseMessage('Invalid resume ID');
      return;
    }
    ...
    // ✅ FIX: only check null, not parsedData
    if (!analysis) {
      setPhase('error');
    ...
    navigation.navigate('ResumeAnalysis', { resumeId });
      ...
      >
        ...
        <View style={styles.pageHeader}>
          ...
          <Text style={styles.pageSubtitle}>
            We'll extract your skills, experience, and match you to the best jobs
          </Text>
        </View>
        ...
        {(phase === 'uploading' || phase === 'uploaded') && (
          <Card style={styles.progressCard} animated>
            <View style={styles.progressFileRow}>
              <View style={styles.progressFileIcon}>
                <Text>📄</Text>
              </View>
              <View style={styles.progressFileInfo}>
                <Text style={styles.progressFileName} numberOfLines={1}>
                  {uploadedResume?.fileName || 'Uploading...'}
                </Text>
                <Text style={styles.progressFileSize}>
                  {uploadedResume?.fileSize || 'Processing...'}
                </Text>
              </View>
        ...
        {phase === 'analyzing' && (
          <Card style={styles.analyzingCard} animated>
            ...
            <Text style={styles.analyzingSubtitle}>
              Our AI is extracting your skills, experience, and finding the best job matches
            </Text>

            <View style={styles.analyzingSteps}>
              {[
                { label: 'Parsing document', done: true },
                { label: 'Extracting skills & experience', done: false },
                { label: 'Generating job matches', done: false },
              ].map((step, i) => (
                <View key={i} style={styles.analyzingStep}>
                  <Text style={styles.analyzingStepIcon}>
                    {step.done ? '✅' : '⏳'}
                  </Text>
                  <Text style={[
                    styles.analyzingStepText,
                    step.done && styles.analyzingStepDone,
                  ]}>
                    {step.label}
                  </Text>
                </View>
...
const infoItems = [
  {
    ...
    desc: 'Identifies technical and soft skills from your resume automatically',
  },
  {
    icon: '📊',
    title: 'Resume Score',
    desc: 'Get a quality score and actionable improvements',
  },
  {
    icon: '💼',
    title: 'Job Recommendations',
    desc: '6 tailored job roles with relevance scores and salary ranges',
  },
  {
    icon: '🎯',
    title: 'Skill Gap Analysis',
    desc: 'See what skills to develop for your target roles',
  },
];
